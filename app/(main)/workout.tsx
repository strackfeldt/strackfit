            ...
            template?.exercises
                .map((exercise) => {
                    ...
                    return {
                        ...
                        id: exerciseData?.id,
                        logs:
                            currentWorkout.logs?.filter(
                                (log) => log.exerciseId === exerciseData.id
                            ) ?? [],
                        previousLogs: [],
                    };
                })
...
function Exercise({
    ...
    exercise: Omit<Template["exercises"][number], "id"> & {
        id: string;
        logs: any[];
        previousLogs: any[];
    };
    ...
    return (
        <>
            ...
            <View className="flex-row justify-between items-center mt-2 mb-1 border-b border-zinc-800 pb-2">
                ...
                <Text className="text-white w-4 flex items-center justify-center">
                    <Feather name="target" size={12} />
                </Text>
            ...
            {exercise.sets.map((set, index) => {
                const previousLog = prevlogs.data?.find(
                    (log) => log.exercise === exercise.id && log.set_nr === index + 1
                );
                ...
                const disabled =
                    !currentLog && index !== logs?.filter((l) => l.exercise === exercise.id).length;
    ...
    return (
        <>
            <View className="flex-row justify-between mt-2">
                ...
                <View className="flex-row items-center justify-center w-4">
                    {(!currentLog ||
                        currentLog.weight !== parseValue(weight) ||
                        currentLog.reps !== parseInt(reps)) && (
                        <TouchableOpacity
                            className={clsx(
                                "w-6 h-6 rounded bg-zinc-800 flex items-center justify-center",
                                disabled && "opacity-50"
                            )}
                            onPress={() => {
                        ...
                        >
                            <Feather
                                name={currentLog ? "refresh-cw" : "check"}
                                size={10}
                                color="gray"
                            />
                        </TouchableOpacity>
